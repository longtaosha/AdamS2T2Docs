...
*/
        ...
        public async Task wordCopyAndDelete(bool isToStreamtext, bool isFilterCN) //defalut lastLength was given "0".
        {
            ...
            int cuttingPoint = -1; 
            int chunkSize = 5; // Set the chunk size for processing the string

            if (str.Contains("**"))
            {
                copiedWordFromDoc = "Doc is in Editing Mode";
                // Trigger the event to update the RichTextBox
                OnTextCopied?.Invoke(copiedWordFromDoc);

                isWordCopyIsDone = true;
                cuttingPoint = str.IndexOf("**"); // for future feature, only copy letters befor "**"
                return;
            }
            ...
            if (str.Length <= 5) // doc's initial length is 4, (two "=", one "retrun")
                ...
                //string chineseCharactersPattern = @"[\u4e00-\u9fa5\d，。？！：；、“”‘’（）—《》〈〉【】『』「」]+";

                // Remove Chinese characters
                // string result = Regex.Replace(str, chineseCharactersPattern, "");
                ...
                // adjust the speed based the bulk volume
                // Regular expression pattern for Chinese characters
                //string chineseCharactersPattern = @"[\u4e00-\u9fa5]";
                //string englishSentencePattern = @"[A-Za-z0-9\s,.'"";:?!()-]+|。";
                // string chineseCharactersPattern = @"[\u4e00-\u9fa5\d，。？！：；、“”‘’（）—《》〈〉【】『』「」]+";             

                // Remove Chinese characters
                //str = Regex.Replace(str, chineseCharactersPattern, "");
                ...
                if (str.Length > 500)
                {
                    ...
                    { 
                        chunkSize = 10;
                        if (str.Length > 3000) chunkSize = 15; 
                    }
                } else chunkSize = 5; 
                ...
                for (int i = 0; i < str.Length; i = i + chunkSize) 
                    {
                       //output the str at once, for it's less than chunk size
                        if (i + chunkSize > str.Length)
                        {
                            if (i < str.Length) // make sure  the next round it won't be trigged, next round i is larger than str length
                            {
                        ...
                        else //  i+chunkSize <=str.Length
                        {
                            await Task.Delay(typeEffect);
                            if (!isWordCopyShouldStop)
                            {
                                bool isBackspace = false; // for streamtext delet effect
                                strForSteamText = str.Substring(i, chunkSize);
                                char randomLetter = ' ';

                               // delete effect preparation
                                if (i == 20 || (i % 20 == 0 && i > 0)) // i == 20, str.length = 100, make a delete efect
                                {
                                    Random random = new Random();
                                    int asciiValue = random.Next(65, 91);  // ASCII values for uppercase letters range from 65 to 90
                                    randomLetter = (char)asciiValue;
                                    strForSteamText = str.Substring(i, chunkSize) + randomLetter + str.Substring(i, 5).ToUpper();
                                    isBackspace = true;
                                }
                                ...
                                {
                                    InsertText = new InsertTextRequest()
                                    { Text = (i == 20 || (i % 200 == 0 && i > 0)) ? str.Substring(i, chunkSize) + randomLetter + str.Substring(i, 5).ToUpper() : str.Substring(i, chunkSize), EndOfSegmentLocation = new EndOfSegmentLocation() }
                                });

                            //stimulate the keyboard
                            //Clipboard.SetText((i == 20 || (i % 200 == 0 && i > 0)) ? str.Substring(i, chunkSize) + randomLetter + str.Substring(i, 5).ToUpper() : str.Substring(i, chunkSize));
                            //Clipboard.SetText( str.Substring(i, chunkSize)+ '\u00A0'); // Add a non-breaking space at the end, otherwise space at the end will be removed
                            //SendKeys.SendWait("^(v)");
                            ...
                            if (i == 20 || (i % 200 == 0 && i > 0)) //trying to make delete effect, diidn't work with sending backspace, has to use DeleteContentRange 
                            {
                                    await Task.Delay(typeEffect + 300);
                                    try
                                    {
        ...
        private string filterChinese( string line)
        {

            string pattern = @"([\u4e00-\u9fa5]+\d+|\d+[\u4e00-\u9fa5]+)"; // Chinese + Number OR Number + Chinese
            line = Regex.Replace(line, pattern, "");
            pattern = @"[A-Za-z]+[。，？]";

            // Use a MatchEvaluator to replace the punctuation in each match
            line = Regex.Replace(line, pattern, match =>
            {
                // Get the matched value
                string matchedValue = match.Value;

                // Replace the full-width punctuation with half-width equivalents
            ...
            pattern = @"[\u4e00-\u9fa5。，？！《》“”‘’（）、；：]+"; //Chinese characters
            line = Regex.Replace(line, pattern, "");
